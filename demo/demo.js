document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('demo.json')
        ...
        .catch(error => {
            ...
            const bookmarksContainer = document.getElementById('bookmarksContainer');
            bookmarksContainer.textContent = 'Failed to load bookmarks.';
        });
    ...
    function displayBookmarks(nodes, container, level = 1) {
        nodes.forEach(node => {
            if (node.children && node.children.length > 0) {
                ...
                folderLink.href = level === 1 ? 'demo.html' : '?start=' + node.id;
...
document.getElementById('settings').onsubmit = function () {
    ...
    sessionStorage.setItem('bgColor', document.getElementById('bg_color').value);
    sessionStorage.setItem('titleColor', document.getElementById('title_color').value);
    sessionStorage.setItem('headingColor', document.getElementById('heading_color').value);
    sessionStorage.setItem('linkColor', document.getElementById('link_color').value);
    sessionStorage.setItem('titleFont', document.getElementById('title_font').value);
    sessionStorage.setItem('headingFont', document.getElementById('heading_font').value);
    sessionStorage.setItem('linkFont', document.getElementById('link_font').value);
    sessionStorage.setItem('columnSize', document.getElementById('column_size').value);
...
function setStyles() {
    ...
    if (bgColor) {
        r.style.setProperty('--user-background-color', bgColor);
        document.getElementById('bg_color').value = bgColor;
        r.style.setProperty('--scrollbar-color', lightenOrDarkenColor(bgColor, 27));
    } else {
    ...
    if (titleColor) {
        r.style.setProperty('--user-title-color', titleColor);
    ...
    const headingColor = sessionStorage.getItem('headingColor');
    if (headingColor) {
        r.style.setProperty('--user-heading-color', headingColor);
        document.getElementById('heading_color').value = headingColor;
    } else {
        r.style.setProperty('--user-heading-color', '#52ff94');
        document.getElementById('heading_color').value = '#52ff94';
    }

    const linkColor = sessionStorage.getItem('linkColor');
    if (linkColor) {
        r.style.setProperty('--user-link-color', linkColor);
    ...
    const titleFont = sessionStorage.getItem('titleFont');
    if (titleFont) {
        r.style.setProperty('--user-title-font', titleFont);
        document.getElementById('title_font').value = titleFont;
    } else {
    ...
    const headingFont = sessionStorage.getItem('headingFont');
    if (headingFont) {
        r.style.setProperty('--user-heading-font', headingFont);
        document.getElementById('heading_font').value = headingFont;
    } else {
    ...
    const linkFont = sessionStorage.getItem('linkFont');
    if (linkFont) {
        r.style.setProperty('--user-link-font', linkFont);
        document.getElementById('link_font').value = linkFont;
    } else {
        r.style.setProperty('--user-link-font', 'Noto Serif');
        document.getElementById('link_font').value = 'Noto Serif';
    }

    const columnSize = sessionStorage.getItem('columnSize');
    if (columnSize) {
        r.style.setProperty('--user-column-size', columnSize + 'px');
        document.getElementById('column_size').value = columnSize;
    } else {
